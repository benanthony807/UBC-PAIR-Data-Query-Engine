import PerformQueryHelperQuery from "./controller/PerformQueryHelperQuery";
import Dataset from "./controller/Dataset";

export default class PerformQueryHelperQueryHelper extends PerformQueryHelperQuery {

    public doOrder (unsortedListOfSections: any): any[] {
        let sortedList: any[];
        let orderKey = this.OrderKey; // ex. courses_avg
        let datasetOrderKey = this.setObjectKeyOfInterest(orderKey);

        sortedList = this.doSort(unsortedListOfSections, datasetOrderKey);
        return sortedList;
    }

    /** Helper function for doOrder: Actual logic for running sort. If tie, run tieBreaker. */
    protected doSort(unsortedListOfSections: any[], datasetOrderKey: string): any {
     ...
     * checks if a key's field exists in key_field
     */
    public doesFieldExist(query: any, currFilterSubType: string): Promise<any> {
        // ex query {"GT": {"courses_avg": 97}}
        let keyField = Object.keys(query[currFilterSubType])[0]; // ex. "courses_avg"
        for (let item in this.listOfAcceptableKeyFields) {
            if (keyField === item) {
                return Promise.resolve();
            }
        }
        return Promise.reject("Invalid key " + keyField + " in " + currFilterSubType);
    }
     ...
     */
    public doesValueTypeMatch(query: any, currFilterSubType: string): Promise<any> {
        /**
         * ex. reached leaf "GT"
        ...
        if (currFilterSubType === "IS") {
            if (typeof queryValue === "string") {
                ...
                return Promise.reject("Invalid value type in " + currFilterSubType + ", should be string");
            }
            // LT GT EQ
        } else {
            if (typeof queryValue === "number") {
                ...
                return Promise.reject("Invalid value type in " + currFilterSubType + ", should be number");
            }
        }
    }
